			...
			try {
				if (pageName.equals("signup")) {
					...
					if (userRepository.findByUserName(emailName)) {
						// request.setAttribute("message", "User "+emailName+" alredy exist. <a
						// href='index.jsp'>Click here</a> to login");
						session.setAttribute("message", "Error. User " + emailName
					...
					if (userRepository.orgExists(orgDomain)) {
						...
						userRepository.saveUserandOrg(request.getParameter("orgName"), request.getParameter("domain"),
								"logoUrl", request.getParameter("firstName"), request.getParameter("lastName"),
								request.getParameter("email"), request.getParameter("phone"), hasp,
								Integer.parseInt(request.getParameter("role")), inputStream, analytics_id);
						System.out.println("New User from new organization");

					}
					forward = USER_LOGIN;
				} else if (pageName.equals("login")) {
					String results = userRepository.findByLogins(request.getParameter("userName"),
							request.getParameter("password"));
					/*
					 * String username = request.getParameter("userName"); String password =
					 * request.getParameter("password");
					 */
					boolean result = userRepository.findByLogin(request.getParameter("userName"),
							request.getParameter("password"));
					if (result == true) {
						...
						if (hasp.equals(array1[1])) {
							...
							session.setAttribute("message",
									"Error: Login failed. Try again with valid login & password.");
					...
					if ((hasps.equals(array1[1]))) {
						...
						userRepository.updateUser(request.getParameter("firstName"), request.getParameter("lastName"),
								request.getParameter("phone"), check, hasp,
								Integer.parseInt(request.getParameter("role")), inputStream, idorg);
						...
						userRepository.updateUserNoPassword(request.getParameter("firstName"),
								request.getParameter("lastName"), request.getParameter("phone"), check,
								Integer.parseInt(request.getParameter("role")), inputStream, idorg);
						int userid = (int) session.getAttribute("user_id");
						User user = userRepository.getUser(userid);
						session.setAttribute("authenticated", "true");
						...
						session.setAttribute("message",
								"Error: Current Password entered is Invalid. Try again with valid  password.");
						forward = "?view=pages/profile-view-myprofile.jsp";
					}
