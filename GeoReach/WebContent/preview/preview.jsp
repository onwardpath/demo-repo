         ...
         function activateDnD()
         {
        	 //Merging two Objects
        	   ...
        	   //extend(a,b);
        	    ...
        	    var treeFrameWindow = $('#treeFrameWindow').get(0).contentWindow;
        	    
        	    $(treeFrameWindow.document.body).find("#dragitemslistcontainer li").on('dragstart',function(event) {
        	        ...
        	        const data = $(this).attr('data-insert-html');
        	        var curr_exp_obj = {};
        	        curr_exp_obj[id]= data;

        	        var getLSobj = JSON.parse(localStorage.getItem("insertingHTML"));
        	        if(getLSobj == null)
        	        {
        	        localStorage.setItem("insertingHTML", JSON.stringify(curr_exp_obj));
        	        }
        	        else
        	        {
        	        var updated_obj = extend(getLSobj,curr_exp_obj);
        	        localStorage.setItem("insertingHTML", JSON.stringify(updated_obj));
        	        }
        	        
        	        event.originalEvent.dataTransfer.setData("Text",insertingHTML);
        	    });
        	    $(treeFrameWindow.document.body).find("#dragitemslistcontainer li").on('dragend',function() {
        	            ...
        	            //   var textData = e.dataTransfer.getData('text');
        	             //Setting Experinece data into another iframe using Localstorage
        	               ...
        	               var textData = div_el.outerHTML; 
        	                ...
        	                var checkDiv = $(textData.toString());
        	                insertionPoint.after(checkDiv);
        	                insertionPoint.remove();
        	                localStorage.removeItem("insertingHTML");
        	            }
        	            catch(e)
        	            {
        	                console.log(e);
...
<body>
	...
	<div class="main">
		...
		<div id="clientframe-container">
			<iframe id="clientframe" class="external-page" frameborder="0"></iframe>
		</div>
	</div>
